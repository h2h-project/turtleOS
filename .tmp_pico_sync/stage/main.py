...
def step_rtc():
    ...
    if tz_min is not None:
        try:
            ...
            return True, "OK {:02d}:{:02d} local ({}){}".format(lh, lmi, tz_label, _bat)
        except Exception:
            pass

    return True, "OK {:02d}:{:02d} UTC{}".format(h, mi, _bat)
...
def step_warmup():
    ...
    # Build capability list from what's on the bus.
    caps = []
    if has_bme or has_aht:
        caps += ["temp", "hum"]
    if has_bme:
