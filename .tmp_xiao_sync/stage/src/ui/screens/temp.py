...
class TempScreen:
    ...
    def _draw_screen(self, reading, rtc_temp_c=None):
        ...
        # --- Primary temp: BME280 > AHT21 > primary selected value ---
        temp_c = None
        if reading:
            temp_c = getattr(reading, "bme280_temp_c", None)
            if temp_c is None:
        ...
        # --- Bottom-left: Humidity (BME280 > AHT21 > primary selected) ---
        rh = None
        if reading:
            rh = getattr(reading, "bme280_humidity", None)
            if rh is None:
