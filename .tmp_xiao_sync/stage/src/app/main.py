...
def run(
    ...
    def get_screen(name):
        ...
        try:
            ...
            if name == "device":
                ...
                screens[name] = StateScreen(
                    oled,
                    nav=_get_nav(),
                    cfg_get=lambda: _cfg_cell[0],
                )
    ...
    def _get_nav():
        if _nav_cell[0] is not None:
            return _nav_cell[0]
        cfg_now = _cfg_cell[0] or _gps_cfg
        if not cfg_now.get("turtle_mode", False):
            return None
        try:
            _gc()
            from src.nav.controller import NavController
            nav_servo = None
            if cfg_now.get("servo_present", False):
                try:
                    from src.hal.board import servo_pin as _servo_pin
    ...
    # ------------------------------------------------------------
    # Turtle machine state: boot pipeline complete → ACQUIRE.
    # (BOOT is the state_machine module default; any reboot re-enters it.)
    # ------------------------------------------------------------
    if _gps_cfg.get("turtle_mode", False):
        try:
            from src.nav.state_machine import set_state as _nav_set
            from src.nav.state_machine import ACQUIRE as _NAV_ACQUIRE
            _nav_set(_NAV_ACQUIRE, "boot complete")
        except Exception as e:
            print("[NAV] state init failed:", repr(e))
        _get_nav()
    ...
    while True:
        ...
        if turtle_waiting_scr is not None:
            ...
            # SAIL-NAV's 200-500 ms control loop.
        ...
        if action:
            try:
                ...
                if _dest:
                    print("[CLICK] {} -> {}".format(action, _dest))
                else:
                    print("[CLICK]", action)
            except Exception:
                print("[CLICK]", action)
        ...
        if action == "double":
            ...
            if cfg.get("turtle_mode", False):
                ...
                if scr is not None:
                    try:
                        _nav = _get_nav()
                        if _nav and hasattr(_nav, "snapshot"):
                            _snap = _nav.snapshot()
                            print("[SCREEN] state  state={}  heading={}  sail={}  wind={}".format(
                                _snap.get("state", "?"), _snap.get("heading", "?"),
                                _snap.get("sail", "?"), _snap.get("wind", "?")))
                        else:
                            try:
                                from src.nav.state_machine import get_state as _sm_get
                                print("[SCREEN] state  state={}".format(_sm_get()))
                            except Exception:
                                print("[SCREEN] state")
                    except Exception:
                        print("[SCREEN] state")
        ...
        if action == "quint":
            ...
            try:
                ...
                show_toggle_mode(oled, cfg.get("turtle_mode", False))
            except Exception as e:
                print("[QUINT] togglemode failed:", repr(e))
            continue
