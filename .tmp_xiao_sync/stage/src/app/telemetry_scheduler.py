...
class TelemetryScheduler:
    ...
    def _read_gps_fix(self):
        ...
        if delta_rejects > 0:
            ...
            if self._gps_reject_streak >= self._GPS_RELOC_THRESH:
                ...
                self._last_gps_lon = None
                self._gps_reject_streak = 0
        # Fall back to the nav fix cache: in turtle mode NavController
        # drains the same UART every nav cycle, so this buffer is usually
        # empty by the time telemetry looks. Accept fixes < 5 min old.
        try:
            from src.nav.gpsfix import get as _navfix_get
            lat, lon, age_ms = _navfix_get()
            if lat is not None and age_ms is not None and age_ms < 300000:
                return lat, lon
        except Exception:
            pass
        return None, None
    ...
    def _build_full_payload(self, cfg, rtc_dict, do_print=False):
        ...
        WiFi-independent — safe to call whether or not the network is available."""
        ...
        if not got_reading and not (cfg and cfg.get("turtle_mode", False)):
            # airOS: no air reading means nothing worth posting.
            # turtle_mode: sail turtle may carry no air sensor — battery/GPS/
            # machine_state still need to reach the server, so proceed anyway.
            if do_print:
        ...
        _rtc_dict = rtc_dict
        rtc_temp_c = None
        if _rtc_dict is None and self.get_rtc:
            try:
                _rtc_dict = self.get_rtc()
            except Exception:
                _rtc_dict = None
        if isinstance(_rtc_dict, dict):
            rtc_temp_c = _rtc_dict.get("temp_c")
        ...
        try:
            from src.nav.state_machine import get_state as _nav_get_state
            payload["machine_state"] = _nav_get_state()
        except Exception:
            pass
    ...
    def tick(self, cfg, rtc_dict=None):
        ...
        # Build payload from local sensors (WiFi-independent)
        ...
        # Offline: enqueue to flash and return — HTTP fires on next connected tick
        if not wifi_ok:
            if do_print:
                self._dbg_print("telemetry: wifi offline — queuing to flash")
            client.enqueue(payload)
            self.write_last_sent(payload["recorded_at"], ok=False)
            self.api_state["sending"] = False
            self.api_state["ok"] = False
            self.api_state["msg"] = "queued (offline)"
        ...
        self.write_last_sent(payload["recorded_at"], ok=bool(ok))
