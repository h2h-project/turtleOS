...
def _dt_to_unix(y, mo, d, hh, mm, ss):
    """
    Convert UTC components to a Unix timestamp without time.mktime().
    On RP2040, time.mktime() returns Unix epoch (1970); on ESP32 it returns
    MicroPython epoch (2000). Bypassing it avoids the 30-year offset bug.
    """
...
def sync_system_rtc_from_ds3231(i2c, min_year=2020, tz_offset_s=0):
    ...
    try:
        ...
        if year < min_year:
            ...
            out["dt_utc"] = (year, month, day, wd0, hour, minute, sec)
            out["unix"] = _dt_to_unix(year, month, day, hour, minute, sec)
            out["synced"] = False
            return out
...
def sync_system_rtc_from_ntp(timeout_s=5, host=None):
    """
    Sync system RTC from NTP (pool.ntp.org by default).
    Fallback when DS3231 has lost power (OSF set) or is absent.
    ...
    try:
        ...
        out["reason"] = "ntptime_unavailable"
        return out
    ...
    try:
        ...
        try:
            ntptime.timeout = int(timeout_s)
        except Exception:
            pass

        ntptime.settime()  # blocks up to timeout_s; sets machine.RTC() to UTC

        t = time.gmtime()
        y, mo, d, hh, mm, ss, wd = t[0], t[1], t[2], t[3], t[4], t[5], t[6]

        out["ok"] = True
        out["synced"] = True
        out["dt_utc"] = (y, mo, d, wd, hh, mm, ss)
        out["unix"] = _dt_to_unix(y, mo, d, hh, mm, ss)
        return out

    except Exception as e:
        out["error"] = repr(e)
        out["reason"] = "ntp_failed"
        return out