...
class TVOCScreen:
    ...
    def __init__(self, oled):
        ...
        # Fonts (match CO2)
        self.f_arvo16 = getattr(oled, "f_arvo16", None)
        self.f_med = getattr(oled, "f_med", None)
        self.f_large = getattr(oled, "f_large", None)
...
# Layout tuning (match CO2)
        ...
        _, h_vs = self.oled._text_size(self.f_small, "Ag")
    ...
    def _draw_header(self, tvoc, conf_text, not_ready):
        ...
        title_writer = self.f_arvo16 if self.f_arvo16 else self.f_med
        title_writer.write("TVOC", x0, y0)

        # Unit "PPB" in SMALL — same style as CO2
        w_main, _ = title_writer.size("TVOC")
        unit_writer = self.f_small
    ...
    def _draw_bottom_labels(self):
        ...
        for txt, x in zip(self.label_texts, self.label_x):
            self.f_small.write(txt, int(x), int(y_text))
